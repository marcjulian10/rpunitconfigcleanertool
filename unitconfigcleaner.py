...
def contains_special_chars(s):
    ...
    # Regex checks for anything that is NOT alphanumeric, whitespace, or hyphen
...
def review_special_char_rows(df, file_key):
    ...
    decision_key = f"char_decision_{file_key}"

   

    st.warning(f"⚠️ Special characters detected in the 'Unit' column of {len(df)} rows!")

    st.write("Tower values allow special characters, but Units do not. Choose action:")
    ...
    def set_char_choice():
        ...
        st.session_state[f"char_choice_val_{file_key}"] = st.session_state[f"char_radio_{file_key}"]
    ...
    st.radio(
        ...
        key=f"char_radio_{file_key}",

        on_change=set_char_choice

    )



    current_choice = st.session_state.get(f"char_choice_val_{file_key}", "Keep These Rows")
    ...
    if st.session_state[decision_key] is None:
        ...
        st.button("Confirm Special Char Action", key=f"confirm_char_{file_key}", on_click=set_decision)
...
def review_duplicates(df, duplicates_df, file_key):
    ...
    st.write("These rows result in identical Unit IDs. How do you want to handle them?")
...
def clean_units_streamlit(file, file_key):
    ...
    try:
        ...
        # CHECK 1: Special Characters (UNIT COLUMN ONLY)

        # ---------------------------

        # We only check the Unit column. Tower is allowed to have special chars.

        special_char_mask = df[unit_col].astype(str).apply(contains_special_chars)
        ...
        if not problem_rows.empty:
            ...
            if decision == "delete":
                ...
                st.session_state[result_key] = f"🟡 Canceled processing for {file.name} (Special Characters)."
        ...
        # Check for duplicates based on the created 'Unit' column
        ...
        if not duplicates_df.empty:
            ...
            # If "keep_all", we do nothing and proceed
        ...
        if deleted_rows_count > 0:
            ...
            result_message += f"🗑️ Deleted {deleted_rows_count} rows with special characters in Unit.\n"
...
def handle_upload():
    ...
    prefixes = ['result_', 'data_', 'char_decision_', 'char_choice_val_', 'dup_decision_', 'dup_choice_val_', 'output_']
...
if uploaded_files:
    ...
    st.info("Files are processed sequentially. You may be prompted to make decisions for Special Characters (Unit only) and Duplicates.")
